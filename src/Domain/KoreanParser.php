...
class KoreanParser extends AbstractParser
{
  ...
  public static function MeCab_command(string $args): string
  {
    if (!KoreanParser::MeCab_installed()) {
      ...
      throw new \Exception("MeCab-Ko not installed or not on your PATH (OS = {$os})");
    }
    $p = KoreanParser::MeCab_app();
  ...
  private static function MeCab_app(): ?string
  {
    $os = strtoupper(substr(PHP_OS, 0, 3));
    if ($os == 'LIN' || $os == 'DAR') {
      if (shell_exec("command -v /opt/mecab/bin/mecab"))
        return '/opt/mecab/bin/mecab';
    }
    // if ($os == 'WIN') {
  ...
  private function getMecabResult(string $text, string $args)
  {
    ...
    $mecab = KoreanParser::MeCab_command($mecab_args);
